...
#define SHAREBTN_ITEM_SIZE_WIDTH                75
#define SHAREBTN_ITEM_SIZE_HEIGHT               100
...
- (instancetype)initWithTitle:(NSString *)title delegate:(id<QGLShareActivityDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle ShareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray {
    ...
    if (self) {
        ...
        [self creatButtonsWithTitle:self.title cancelButtonTitle:self.cancelButtonTitle shareButtonTitles:self.shareButtonTitlesArray withShareButtonImagesName:self.shareButtonImagesNameArray];
    }
    return self;
}
...
- (UILabel *)titleLabel:(NSString *)title {
    ...
    titleLabel.textColor = [UIColor colorWithRed:0.786 green:0.431 blue:0.367 alpha:0.8];
...
//    }
    ...
    flowLayout.headerReferenceSize = [self titleLabel:self.title].bounds.size;
    ...
    self.shareBtnView = [[UICollectionView alloc] initWithFrame:CGRectMake(SHAREBTN_INTERVAL_WIDTH, y, SHAREBTN_WIDTH, height + [self titleLabel:self.title].bounds.size.height) collectionViewLayout:flowLayout];
    self.shareBtnView.backgroundColor = [UIColor whiteColor];
    self.shareBtnView.delegate = self;
    self.shareBtnView.dataSource = self;
    [self.shareBtnView registerClass:[CustomCollectionViewCell class] forCellWithReuseIdentifier:@"CustomCollectionViewCell"];
    [self.shareBtnView addSubview:[self titleLabel:self.title]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.itemTitle.text = [self.shareButtonTitlesArray objectAtIndex:indexPath.row];
    cell.logoIV.image = [UIImage imageNamed:[self.shareButtonImagesNameArray objectAtIndex:indexPath.row]];
    return cell;
}
