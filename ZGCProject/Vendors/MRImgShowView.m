...
- (void)setAllImgVContentFromImage:(NSArray *)imgList{
    ...
    if (imgList.count) {
        if ([imgList[0] isKindOfClass:urlCls]) {
            ...
            [vLift setImageWithURL:[NSURL URLWithString:imgList[MRImgLocationLeft]]];
            [vCenter setImageWithURL:[NSURL URLWithString:imgList[MRImgLocationCenter]]];
            [vRight setImageWithURL:[NSURL URLWithString:imgList[MRImgLocationRight]]];
        }
    }
